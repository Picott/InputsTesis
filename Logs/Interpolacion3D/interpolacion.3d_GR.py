...
# Filtrar nulos esenciales para GR
df_raw = df_raw.dropna(subset=['GR', 'X', 'Y', 'DEPTH', 'Segmento_ID'])

# ==========================================
# 2. PROCESO DE "BLOCKING" (UPSCALING)
# ==========================================
print("🔨 Realizando Blocking de GR por Segmento...")
# Promediamos el GR por segmento geológico para eliminar ruido de alta frecuencia
...
print("🔄 Calculando modelo 3D de Gamma Ray...")
...
interpolated_values = np.sum(weights * df['GR'].values[idx], axis=1)

# Crear Grid
grid = pv.StructuredGrid(X, Y, Z)
grid["GR"] = interpolated_values

# ==========================================
# 4. VISUALIZACIÓN INTERACTIVA CON SLIDERS (GR)
# ==========================================
print("🎨 Generando interfaz interactiva...")

plotter = pv.Plotter()

# Límite de color dinámico para GR (Percentiles 5 y 95)
p05, p95 = np.percentile(df['GR'], 5), np.percentile(df['GR'], 95)
clim_range = [p05, p95]
print(f"📊 Escala Visual GR: {p05:.1f} - {p95:.1f} API")

# Mapa de colores 'nipy_spectral' es excelente para GR (distingue bien arcillas de arenas)
cmap = "nipy_spectral"

# A) VOLUMEN y POZOS (Fijos)
plotter.add_mesh(grid, scalars="GR", cmap=cmap, clim=clim_range, 
                 opacity=0.10, label="Volumen GR Transparente")

cloud = pv.PolyData(df[['X', 'Y', 'Z']].values)
cloud['GR'] = df['GR']
plotter.add_mesh(cloud, scalars="GR", cmap=cmap, clim=clim_range, 
                 point_size=8, render_points_as_spheres=True, label="Datos Reales (GR)")
...
plotter.add_title("Modelo Gamma Ray Interactivo")
