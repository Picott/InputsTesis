...
# Añadir la interpolación 3D como volumen (ajustar opacidad para ver isosuperficies)
plotter.add_mesh(structured_grid, scalars="Vshale", cmap="viridis", opacity=0.3)
...
# === Cortes verticales (Secciones) ===
x_slice = (x_min + x_max) / 2
y_slice = (y_min + y_max) / 2

slice_x = structured_grid.slice(normal=[1, 0, 0], origin=[x_slice, y_min, z_min])  # Corte vertical en X
slice_y = structured_grid.slice(normal=[0, 1, 0], origin=[x_min, y_slice, z_min])  # Corte vertical en Y

plotter.add_mesh(slice_x, scalars="Vshale", cmap="coolwarm", opacity=1, show_edges=True)
plotter.add_mesh(slice_y, scalars="Vshale", cmap="coolwarm", opacity=1, show_edges=True)

# === Isosuperficies ===
# Encontramos valores representativos dentro del rango de Vshale para isosuperficies
vshale_min, vshale_max = df["Vshale"].min(), df["Vshale"].max()
isosurface_values = np.linspace(vshale_min, vshale_max, num=5)  # Generar 5 isovalores dentro del rango

isosurface = structured_grid.contour(isosurface_values)  # Extraer isosuperficies
plotter.add_mesh(isosurface, opacity=0.6, cmap="plasma")
...
plotter.add_axes()
plotter.add_title("Interpolación 3D de Vshale con Cortes e Isosuperficies")
plotter.show()
